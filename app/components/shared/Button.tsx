...
const Button: React.FC<ButtonProps & React.HTMLProps<HTMLButtonElement>> = ({ variant, children, className, type, loading, size = 'large', ...props }) => {
    const classes = classNames('px-4 py-2  w-full md:w-auto font-semibold transition-background duration-300 uppercase disabled:cursor-not-allowed',
        {
            ...
            'min-w-[18rem]': size === "large",
            'min-w-[12rem]': size === "medium",
            'min-w-[8rem]': size === "small",
        }, className
    )
